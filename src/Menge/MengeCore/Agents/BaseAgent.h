...
namespace Menge {
	...
	namespace Agents {
		...
		public:
			...
			*/
			void halfupdate(float timeStep);
			...
			/*!
			*	@brief		Given individual risk based on the average contact force
			*				and its duration.
			*
			*	This should be overriden by child classes to give unique behaviors.
			*	Each pedestrian model is uniquely defined by how it computes its new
			*	risk and this is the critical class.
			*
			*	Trying to instantiate a BaseAgent will cause an exception to be thrown
			*	when calling this function.
			*/
			...
			Math::Vector2	_pos;
			/*!
			 *	@brief		The 2D position of collision between the agent
			 */
			Math::Vector2	_collisionpos;
			...
			*/
			Math::Vector2   _force;
			/*!
			 *  WCY add: the resultant contact normal force 
			 *  considered the contact push force and the force of the impulse (2D)
			*/
			Math::Vector2   _pushingforce;
			/*!
			*  WCY add: the resultant contact normal force
			*  considered the contact push force and the force of the impulse (2D)
			*/
			float   _pushingscalarforce;
			/*!
			*  WCY add: the contact normal feeling force for risk calculation 
			*/
			float _pushingforcerisk;
			/*!
			 *  WCY add: whether the upright posture 
			*/
			bool _posture;
			/*
			 *  WCY add: the impulse exerted on individuals
			*/
			Math::Vector2   _impulse;
			/*
			*  WCY add: the travel distance due to body leanning forward
			*/
			float _leaningleangth;
			/*
			 *  WCY add: the average contact force
			*/
			float _contactforce;
			/*
			 *  WCY add: the duration time of the average contact force
			*/
			float _contactduration;
			/*
			*  WCY add: the duration time of the interaction contact force
			*/
			float _interactionduration;
			/*
			 *  WCY add: the individual risk
			*/
			float _risk;
			/*
			*  WCY add: the individual elastic recovery coefficient
			*/
			float _resistance;
			/*!
			*	WCY add:	The contact agents to which the agent should respond.
			*
			*	Each pair consists of distance between the agent positions, squared
			*	and the pointer to the neigboring agent.
			*/
			std::vector<NearAgent> _contactAgents;
			/*
			*  WCY add: the id of individuals who are pushed by endogenous impulse
			*/
			std::vector<size_t> _pushAgent;
			/*
			*  WCY add: the endogenous force exerted on this agent during pushing
			*/
			std::vector<Math::Vector2> _pushforce;
			/*
			*  WCY add: the id of individuals who are collided with others
			*/
			std::vector<size_t> _collisionAgent;
			/*
			*  WCY add: the endogenous force exerted on this agent during collision
			*/
			std::vector<Math::Vector2> _collisionforce;
